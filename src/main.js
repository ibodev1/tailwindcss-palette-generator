...
const main = {
  ...
  getters: {
    ...
    colorResult() {
      ...
      return Object.assign(DEFAULT, palette);
    }
  },
  checks: {
    checkObject: (params) => {
      ...
      if (
        ...
        if (!main.checks.checkColor(palette.color)) {
          ...
          throw new Error(
            `'${
              palette.shade
            }' - type: ${typeof palette.shade} It must be of type number.`
          );
        } else if (
          palette.shades &&
          !main.checks.checkObject(palette.shades) === "array"
        ) {
    ...
    checkColor: (color) => {
      var reg =
        /^#([\da-f]{3}){1,2}$|^#([\da-f]{6}){1,2}$|(rgb|hsl)a?\((\s*-?\d+%?\s*,){2}(\s*-?\d+%?\s*,?\s*\)?)(,\s*(0?\.\d+)?|1)?\)/gim;
  ...
  if (main.checks.checkObject(params) === "array") {
    for (let index = 0; index < params.length; index++) {
      ...
      if (main.checks.checkParam(colorPalette)) {
        ...
        palette[colorPalette.name] = Object.assign(result, result);
      }
    }
    return palette;
    ...
    if (main.checks.checkParam(params)) {
      ...
      palette[params.name] = Object.assign(result, result);
    ...
    palette[main.state.initialParams.name] = Object.assign(result, result);
